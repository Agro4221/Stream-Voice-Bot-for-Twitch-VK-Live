...
> ℹ️ **Про субтитры:** основной сценарий — русская речь на стриме → T-one STT → отдельные OBS-дорожки оригинала и переводов. Для переводных дорожек языковые пакеты Argos Translate подготавливаются по мере необходимости.
>
> ⚠️ Первый запуск STT и подготовка переводчиков могут занять время. Если CUDA недоступна, бот сразу продолжает на CPU без обязательной загрузки большого CUDA runtime.
>
...
STT реализован через **T-one + sherpa-onnx** — потоковую CTC-модель для русского языка. T-one работает с 8 kHz входом и создана именно для streaming ASR; sherpa-onnx поддерживает Windows x64 и CUDA execution provider.

В проекте используется автоматический backend:

```text
NVIDIA GPU доступна
        ↓
T-one → sherpa-onnx → CUDA

CUDA не запустилась / GPU нет
        ↓
T-one → sherpa-onnx → CPU
```

Пользователю не нужно переключать GPU/CPU вручную. На NVIDIA-системах приложение сначала пытается использовать CUDA, а при проблеме запускает **тот же T-one на CPU**.

### Почему T-one вместо Whisper

Старый STT использовал `faster-whisper` и `large-v3-turbo`. Для live-субтитров это давало лишнюю нагрузку и иногда требовало дополнительной фильтрации Whisper hallucinations, включая характерные строки вроде `Subtitles made by ...`.

T-one использует streaming CTC-декодирование вместо autoregressive Whisper-пайплайна. Поэтому STT теперь получает новые аудиоданные небольшими порциями и не запускает большую модель заново на каждом двухсекундном окне.

Фильтр подозрительных credit-фраз всё равно оставлен как **защита второго уровня**, а не как основной механизм исправления.

### GPU

Для NVIDIA используется `sherpa-onnx` с **CUDA 12.8 + cuDNN 9**. Проект устанавливает CUDA-enabled wheel на NVIDIA-системах; без NVIDIA остаётся CPU wheel. Официальные Windows x64 CUDA wheels доступны для Python 3.11–3.13.

Если локальные CUDA/cuDNN DLL отсутствуют или CUDA не стартует, приложение может один раз подготовить локальный runtime в:

```text
data/gpu_runtime
```

После этого STT использует уже подготовленные DLL и не требует установки полного CUDA Toolkit.

### Streaming

T-one работает непосредственно через `OnlineRecognizer`:
...
Старый аудиобэклог по-прежнему ограничен: при отставании бот не пытается бесконечно догонять старое аудио, а продолжает обрабатывать свежий поток.

### Настройки STT

В админке остаются только практические параметры:

- язык;
- выбранный микрофон;
- запуск/остановка STT.

Выбор модели, beam size, chunk/overlap и GPU/CPU убран из пользовательского интерфейса, потому что эти параметры относятся к внутреннему pipeline и не должны превращать настройку субтитров в технический бенчмарк.
